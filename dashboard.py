...
def load_data(file_bytes: bytes, filename: str) -> pd.DataFrame:
    ...
    df["요일"] = df["기간_일자"].dt.dayofweek
    df["주차번호"] = df["기간_일자"].dt.isocalendar().week.astype(int)
...
def _render_monthly_section(df_tab, targets, tab_key, sameday=False):
    ...
    if not has_target:
        tbl = tbl.drop(columns=[c for c in tbl.columns if "목표" in c])
...
# 페이지 5: 주차별 성과
# ───────────────────────────────────────────────
def page_weekly(df: pd.DataFrame, targets: dict):
    st.header("📅 주차별 성과")
    if df.empty:
        st.warning("필터 조건에 해당하는 데이터가 없습니다.")
        return
...
def page_daily(df: pd.DataFrame, targets: dict):
    ...
    if show_prev and not daily_prev_raw.empty:
        daily_prev_raw = daily_prev_raw.copy()
