...
def calc_kpi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["회원UV비중"] = safe_div(d["지표_UV(회원)"],                       d["지표_UV(전체)"])
    return d
...
def metric_trend_fig(df: pd.DataFrame, val_col: str, gran: str, title: str,
    ...
    cumulative=True: 볼륨 지표를 일평균으로 나누지 않고 해당 기간(월/주) 자체의
    '합계값'으로 표시한다. 비율 지표(ROAS·CTR·CPA·객단가 등)는 기간 집계값 그대로다.
    (일 단위는 이미 일자별 합계이므로 두 모드가 동일하다.)"""
    lbl_fmt = tickfmt if tickfmt else ",.0f"
    is_vol = val_col in VOLUME_COLS
    # 누적(=기간 합계) 모드에서는 일평균(÷집행일수) 환산을 건너뛴다.
    to_avg = is_vol and not cumulative
    if gran == "월":
        d = agg(df, ["연도", "월"]).sort_values(["연도", "월"])
        if to_avg and "집행일수" in d.columns:
        ...
        # (일자별 값은 이미 합계이므로 cumulative 여부와 무관하게 동일하다.)
        ...
        # 주 / 주(최근10주): 주 단위 일평균 (누적 모드면 주 합계)
...
def _sum_kpi_series(series_list):
    """여러 집계 Series의 AGG_COLS를 합산 후 KPI 재계산한 Series 반환(TOTAL 행용)."""
    cols = [c for c in AGG_COLS]
    acc = pd.Series(0.0, index=cols)
    ...
    return calc_kpi(pd.DataFrame([acc])).iloc[0]


def _sum_targets(tgt_list):
    """월/주 목표 dict 합산(비율은 재계산)."""
    sp = sum((t or {}).get("spend", 0) or 0 for t in tgt_list)
    rv = sum((t or {}).get("rev", 0) or 0 for t in tgt_list)
    return {"spend": sp, "rev": rv, "roas": (rv / sp if sp else 0)}


def summary_table(rows, metric_labels, groups, period_type: str = "월",
                  add_total: bool = False) -> pd.DataFrame:
    ...
    add_total: True면 맨 아래 TOTAL 행(입력 rows 합산) 추가
    ...
    if add_total and rows:
        cur_tot = _sum_kpi_series([r[1] for r in rows])
        prev_tot = _sum_kpi_series([r[2] for r in rows]) if any(r[2] is not None for r in rows) else None
        tgt_tot = _sum_targets([r[3] for r in rows])
        rows = rows + [("TOTAL", cur_tot, prev_tot, tgt_tot, False)]
...
def detail_table(rows, period_label="기간", add_total=True) -> pd.DataFrame:
    """실적 상세표. rows: [(라벨, cur_series)]. add_total=True면 TOTAL 행 추가."""
    rows = list(rows)
    data = []
    for label, s in rows:
        ...
        data.append(rec)
    if add_total and rows:
        tot = _sum_kpi_series([s for _, s in rows])
        rec = {period_label: "TOTAL"}
        for disp, col, kind in DETAIL_SPEC:
            rec[disp] = _fmt_kind(tot.get(col, np.nan), kind)
        data.append(rec)
    return pd.DataFrame(data, columns=[period_label] + DETAIL_COLS)


def detail_table_yoy(rows, period_label="기간", add_total=True) -> pd.DataFrame:
    ...
    if add_total and rows:
        ...
        rec = {period_label: "TOTAL"}
        for disp, col, kind in DETAIL_SPEC:
            c = cur_tot.get(col, np.nan)
            p = prev_tot.get(col, np.nan) if prev_tot is not None else np.nan
            rec[disp] = ("–" if (pd.isna(c) or pd.isna(p) or p == 0)
                         else signed_pct((c - p) / abs(p)))
        data.append(rec)
    return pd.DataFrame(data, columns=[period_label] + DETAIL_COLS)
...
def _hl_total_row(row):
    """첫 컬럼이 'TOTAL'인 행을 굵게+음영 강조(Styler.apply axis=1용)."""
    return (["font-weight:700;background-color:#EEF2FF"] * len(row)
            if str(row.iloc[0]) == "TOTAL" else [""] * len(row))


def _style_summary(tbl, metric_labels, groups):
    """전년비·목표비 컬럼 색상 + TOTAL 행 강조."""
    subset = [(g, m) for g in ("전년비", "목표비") if g in groups for m in metric_labels]
    sty = tbl.style
    if subset:
        sty = sty.map(chg_style, subset=subset)
    return sty.apply(_hl_total_row, axis=1)
...
def _render_trend_grid(df, targets, src="TOTAL"):
    ...
    if cumulative:
        st.caption("월별 **합계**(볼륨 지표는 월 합계값). 당월은 전년도 **MTD**(동요일 -364일) 창으로 비교합니다.")
    else:
...
def _render_period_section(df_tab, gran, tab_name, weekly_targets=None, monthly_targets=None,
    ...
    def _show(rows):
        tbl = summary_table(rows, metric_labels, groups, period_type=ptype, add_total=True)
        st.dataframe(_style_summary(tbl, metric_labels, groups),
                     use_container_width=True, hide_index=True,
...
def _render_period_graph(df, gran, key_prefix, prev_df=None, src="TOTAL"):
    ...
    '누적으로 보기' 토글로 일평균(일 단위는 일자값) ↔ 기간 합계값 보기를 전환한다."""
    cumulative = st.toggle(
        "📊 누적으로 보기", value=False, key=f"{key_prefix}_cumulative",
        help="켜면 일평균 대신 해당 기간(주/월)의 '합계값'으로 표시합니다. "
             "비율 지표(ROAS·CTR·CPA·객단가 등)는 기간 집계값 그대로이며, "
             "일 단위는 이미 일자별 합계라 변화가 없습니다.")
    if cumulative:
        mode_note = "" if gran == "일" else " · 합계"
    else:
    ...
    if cumulative:
        suffix = "" if gran == "일" else f" ({gran} 합계)"
    else:
...
def _render_period_detail(df, gran, key_prefix="", prev_df=None, src="TOTAL"):
    ...
    def _actual(rows):
        t = detail_table([(l, s) for l, s, _, _ in rows], period_label=ptype)
        st.dataframe(t.style.apply(_hl_total_row, axis=1),
                     use_container_width=True, hide_index=True,
                     height=_fit_height(len(rows) + 1))

    def _yoy(rows):
        t = detail_table_yoy([(l, s, p, b) for l, s, p, b in rows], period_label=ptype)
        st.dataframe(t.style.map(chg_style, subset=DETAIL_COLS).apply(_hl_total_row, axis=1),
                     use_container_width=True, hide_index=True,
                     height=_fit_height(len(rows) + 1))
...
def render_group_sheet_body(df, group_col, order=None, ptype=None, prev_source=None):
    ...
    nrow = len(cur)
    st.markdown("##### 📇 실적 요약")
    tbl = summary_table(rows, DEFAULT_METRICS, ("실적", "전년비"), period_type=ptype, add_total=True)
    st.dataframe(_style_summary(tbl, DEFAULT_METRICS, ("실적", "전년비")),
                 use_container_width=True, hide_index=True,
                 height=_fit_height(nrow + 1, header_rows=2))
    st.caption("ℹ️ 전년비는 동요일 기준(-364일) 동기간 비교입니다.")

    st.markdown("##### 📄 상세 실적")
    dt = detail_table([(str(r[group_col]), r) for _, r in cur.iterrows()], period_label=ptype)
    st.dataframe(dt.style.apply(_hl_total_row, axis=1),
...
# 캠페인/하위캠페인 랭킹 표 전용 비용출처 그룹(전역 COST_BUCKETS와 별개)
RANK_BUCKETS = {
    "거래액확대": ["거래액확대", "E영업/광고주직접정산", "E영업/정산제외", "서비스비용-거래액확대"],
    "신규고객확대": ["신규고객확대", "신규고객확대-실적구분", "서비스비용-신규고객확대"],
    "인지도제고": ["인지도제고", "인지도제고/브랜딩"],
}
...
def render_ranking_table(df, group_col, src_name, sort_label, sort_col,
    ...
    n_txt = "전체" if not top_n else f"상위 {top_n}개"
    prefix = f"{label}×매체별" if len(group_cols) > 1 else f"{label}별"
    st.markdown(f"##### {prefix} 실적 {n_txt} ({src_name}) · {sort_label} 정렬")
...
def page_campaign(df: pd.DataFrame, targets: dict = None):
    ...
    with tab_rank:
        ...
        with cc2:
            topn_sel = st.selectbox("표시 개수", ["상위 50개", "상위 100개", "전체"],
                                    key="camp_topn")
        top_n = {"상위 50개": 50, "상위 100개": 100, "전체": None}[topn_sel]
...
# ───────────────────────────────────────────────
# 펼칠 수 있는 차원(표기 → 그룹 기준). 기간(일/주/월)도 하나의 단계로 넣을 수 있다.
# 순서는 사용자가 '단계별 선택칸'으로 자유 지정한다.
DRILL_DIM_OPTS = {
    "채널": "구분_채널",
    "매체": "구분_매체명",
    "상품": "구분_상품",
...
def _drill_series(df_sub, col):
    """그룹 기준 Series. 기간 차원(__day__/__week__/__month__)은 파생 키로."""
    if col == "__day__":
        return df_sub["기간_일자"].dt.strftime("%Y-%m-%d")
    if col == "__week__":
        return df_sub["기간_주"].astype("object").astype(str)
    if col == "__month__":
        return df_sub["연월"].astype("object").astype(str)
    return df_sub[col]
...
def _drill_build_tree(df, dims, top_n, impr_only=False, sort_by="광고비", daily_avg=False,
    ...
    # 캠페인·하위캠페인 단계에는 기획전번호를 이름 앞에 붙인다(단일값일 때).
    promo_on = _DRILL_PROMO_COL in df.columns
    promo_prefix_cols = {"구분_캠페인", "구분_하위캠페인"}
...
def _drill_export_df(nodes, dims, spec, show_promo=False):
    """트리 노드를 CSV용 표로. [기획전번호] + 단계별 차원 컬럼(조상 경로) + 지표 숫자."""
    dim_labels, seen = [], set()
...
<style>
  ...
  tr.d2 td,tr.d2 td.name{background:#ffffff}
  tr.d3 td,tr.d3 td.name{background:#f4f5f7}
  tr.d4 td,tr.d4 td.name{background:#ffffff}
  tr.d5 td,tr.d5 td.name{background:#f4f5f7}
  ...
  @media (prefers-color-scheme:dark){
    ...
    tr.d3 td,tr.d3 td.name{background:#212121}
    tr.d4 td,tr.d4 td.name{background:#1a1a19}
    tr.d5 td,tr.d5 td.name{background:#212121}
...
def page_drilldown(df: pd.DataFrame, targets: dict = None, report_targets: dict = None):
    st.header("🌳 펼쳐보기 실적")
    st.caption("전체에서 시작해 **이름을 클릭하면 그 아래 단계가 펼쳐지고, 다시 클릭하면 접힙니다.** "
               "펼칠 단계와 순서는 아래에서 자유롭게 정하세요(기간도 한 단계로 넣을 수 있어요).")
    if df.empty:
        st.warning("데이터가 없습니다.")
        return
    ...
    for i in range(DRILL_SLOTS):
        d = DRILL_DEFAULT[i] if i < len(DRILL_DEFAULT) else DRILL_NONE
        sel = slot_cols[i].selectbox(
            f"{i + 1}단계", opts,
            index=opts.index(d) if d in opts else 0, key=f"drill_slot_{i}")
...
def page_custom(df: pd.DataFrame, targets: dict = None, report_targets: dict = None):
    ...
    st.caption("🧷 **피벗 행처럼** 보기: ‘행 차원’에 넣은 순서가 곧 **행 중첩 순서**입니다"
               ...
               "**기간을 맨 뒤(맨 안쪽)에 두면** 각 그룹(예: 하위캠페인) 안에서 "
               "날짜순(화·수·목…)으로 자동 정렬됩니다. "
               "⚠️ 표의 **열 머리글 클릭은 ‘전체’ 기준 즉석 정렬**이라 그룹 묶음이 풀립니다 — "
    ...
    if group_cols and not table.empty:
        ...
        trow[list(out.keys())[0]] = "TOTAL"
        trow_raw[list(raw.keys())[0]] = "TOTAL"
        for m in mets:
            if m not in out:
                continue
            _, col, kind = spec_by_label[m]
